...
@implementation RLHttpHint
+ (instancetype)shareInstance{
    
    static RLHttpHint *alert;
    static dispatch_once_t onceTocken;
    dispatch_once(&onceTocken ,^{
        alert = [[RLHttpHint alloc] init];
    });
    return alert;
}
...
void ShowSuccessStatus(NSString *statues){
    if (![NSThread isMainThread]) {
        dispatch_async(dispatch_get_main_queue(), ^{
            RLBubbleInfo * info = [[RLHttpHint shareInstance] getDefaultRightBubbleInfo];
            info.title = statues;
            [[RLBubbleView defaultBubbleView] showWithInfo: info autoCloseTime: 2.5];
        });
    }else{
        RLBubbleInfo * info = [[RLHttpHint shareInstance] getDefaultRightBubbleInfo];
        info.title = statues;
...
void ShowMessage(NSString *statues){
    ...
    if (![NSThread isMainThread]) {
        dispatch_async(dispatch_get_main_queue(), ^{
            RLBubbleInfo *iconInfo = [[RLBubbleInfo alloc] init];
            ...
            iconInfo.title = statues;
            iconInfo.proportionOfDeviation = 0.05;
            iconInfo.bubbleSize = CGSizeMake(300, 60);
            [[RLBubbleView defaultBubbleView] showWithInfo: iconInfo autoCloseTime: 2];
        });
    }else{
        RLBubbleInfo *iconInfo = [[RLBubbleInfo alloc] init];
        // 把图标数组里面设置只有一张图片即可单图固定图标
...
void ShowErrorStatus(NSString *statues){
    if (![NSThread isMainThread]) {
        dispatch_async(dispatch_get_main_queue(), ^{
            RLBubbleInfo *info = [[RLHttpHint shareInstance] getDefaultErrorBubbleInfo];
            info.title = statues;
            [[RLBubbleView defaultBubbleView] showWithInfo: info autoCloseTime: 2.5];
        });
    }else{
...
void ShowMaskStatus(NSString *statues){
    if (![NSThread isMainThread]) {
        dispatch_async(dispatch_get_main_queue(), ^{
            RLBubbleInfo *info = [[RLHttpHint shareInstance] getDefaultRoundProgressBubbleInfo];
            info.title = statues;
            [[RLBubbleView defaultBubbleView] showWithInfo: info];
        });
    }else{
        RLBubbleInfo *info = [[RLHttpHint shareInstance] getDefaultRoundProgressBubbleInfo];
        info.title = statues;
...
void DismissHud(void){
    if (![NSThread isMainThread]) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [[RLBubbleView defaultBubbleView] hide];
        });
    }else{
        [[RLBubbleView defaultBubbleView] hide];
    }
...
- (RLBubbleInfo *)getDefaultRightBubbleInfo{
    RLBubbleInfo *info = [[RLBubbleInfo alloc] init];
...
- (RLBubbleInfo *)getDefaultRoundProgressBubbleInfo{
    RLBubbleInfo *info = [[RLBubbleInfo alloc] init];
    info.iconAnimation = ^(CAShapeLayer *layer){
...
- (RLBubbleInfo *)getDefaultErrorBubbleInfo{
    RLBubbleInfo *info = [[RLBubbleInfo alloc] init];
    info.iconAnimation = ^(CAShapeLayer *layer){
        const int STROKE_WIDTH = 3;// 默认的划线线条宽度
